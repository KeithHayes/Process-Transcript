...
class ParseFile:
    ...
    def generateIOpair(self, chunk, formatted):
        ...
        entry = {"instruction": command,"input": input_text,"output": output_text}
        ...
        if not os.path.exists(TRAINING_FILE):
            ...
            with open(TRAINING_FILE, 'w', encoding='utf-8') as f:
                f.write(json_str + '\n')  # Newline only after complete JSON
        else:
            # Append new entry (with newline) if not a duplicate
            with open(TRAINING_FILE, 'r+', encoding='utf-8') as f:
                content = f.read()
                if json_str not in content:
                    f.write(json_str + '\n')  # Newline only after complete JSON
